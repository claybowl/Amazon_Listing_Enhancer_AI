...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, numberOfImages = 1, modelId = "stability/sdxl" } = body
...
Generate an image that makes this specific product look highly desirable in its new fashionable environment.`
    ...
    for (const result of results) {
      if (result.error) {
        return NextResponse.json(
          { error: result.error.message || "Failed to generate images with OpenRouter" },
          { status: 500 },
        )
      }
    }
    ...
    if (images.length === 0) {
      return NextResponse.json({ error: "No images generated by OpenRouter." }, { status: 500 })
    }
